...
export default function SimulationPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/tdcsHeadData/simnibs_mesh_data_XYZ_${fileIndices.xyzNum}.json`).then((res) => res.json()),
      fetch(`/tdcsHeadData/simnibs_mesh_data_VE_${fileIndices.veNum}.json`).then((res) => res.json())
    ])
    .then(([xyzData, veData]) => {
